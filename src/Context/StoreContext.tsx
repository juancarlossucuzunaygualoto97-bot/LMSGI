...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  async function cargarCursos() {
    ...
    if (error) { console.error("No se pudieron cargar los cursos", error); return; }
    setCursos(data || []);
  }

  async function cargarServicios() {
    const { data, error } = await supabase.from("servicios").select("*").order("id", { ascending: false });
    if (error) { console.error("No se pudieron cargar los servicios", error); return; }
    setServicios(data || []);
  }

  async function cargarTrabajos() {
    const { data, error } = await supabase.from("trabajos").select("*").order("id", { ascending: false });
    if (error) { console.error("No se pudieron cargar los trabajos", error); return; }
    setTrabajos(data || []);
  }

  async function cargarFormacion() {
    const { data, error } = await supabase.from("formacion").select("*").order("id", { ascending: false });
    if (error) { console.error("No se pudo cargar la formación", error); return; }
    setFormacion(data || []);
  }
  ...
  async function addCurso(curso: Omit<ICurso, "id">) {
    const { data, error } = await supabase.from("cursos").insert({
      ...curso,
      tags: Array.isArray(curso.tags)
        ? curso.tags
        : (curso.tags as unknown as string)?.split(",").map((t: string) => t.trim()) || [],
      fecha: curso.fecha || new Date().toISOString(),
    }).select().single();
    if (error) { console.error("No se pudo guardar el curso", error); return; }
  ...
  async function addServicio(servicio: Omit<IServicio, "id">) {
    const { data, error } = await supabase.from("servicios").insert({
      ...
      caracteristicas: Array.isArray(servicio.caracteristicas)
        ...
        : (servicio.caracteristicas as unknown as string)?.split(",").map((c: string) => c.trim()) || [],
      fecha: servicio.fecha || new Date().toISOString(),
    }).select().single();
    if (error) { console.error("No se pudo guardar el servicio", error); return; }
  ...
  async function addTrabajo(trabajo: Omit<ITrabajo, "id">) {
    const { data, error } = await supabase.from("trabajos").insert({
      ...trabajo,
      tags: Array.isArray(trabajo.tags)
        ? trabajo.tags
        : (trabajo.tags as unknown as string)?.split(",").map((t: string) => t.trim()) || [],
      fecha: trabajo.fecha || new Date().toISOString(),
    }).select().single();
    if (error) { console.error("No se pudo guardar el trabajo", error); return; }
  ...
  async function addFormacion(item: Omit<IFormacion, "id">) {
    ...
    if (error) { console.error("No se pudo guardar la formación", error); return; }
    if (data) setFormacion((prev) => [data, ...prev]);
  }

  async function deleteCurso(id: number) {
    const { error } = await supabase.from("cursos").delete().eq("id", id);
    if (error) { console.error("No se pudo borrar el curso", error); return; }
    setCursos((prev) => prev.filter((c) => c.id !== id));
  }

  async function deleteServicio(id: number) {
    const { error } = await supabase.from("servicios").delete().eq("id", id);
    if (error) { console.error("No se pudo borrar el servicio", error); return; }
    setServicios((prev) => prev.filter((s) => s.id !== id));
  }

  async function deleteTrabajo(id: number) {
    const { error } = await supabase.from("trabajos").delete().eq("id", id);
    if (error) { console.error("No se pudo borrar el trabajo", error); return; }
    setTrabajos((prev) => prev.filter((t) => t.id !== id));
  }

  async function deleteFormacion(id: number) {
    const { error } = await supabase.from("formacion").delete().eq("id", id);
    if (error) { console.error("No se pudo borrar la formación", error); return; }
